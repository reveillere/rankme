...
import { listCrosscheckDecisions, importLocalDecisions, removeCrosscheckDecision, personalDataFile, reportPublicationKeys } from '../personalData';
...
export function CrosscheckDecisionsButton({ report, scope = { type: 'all' } }) {
  ...
  return <>
    <Button size="small" variant="outlined" onClick={() => setOpen(true)}>{scope.type === 'all' ? title : 'My cross-check decisions'}</Button>
    <Dialog open={open} onClose={() => setOpen(false)} maxWidth="md" fullWidth>
      ...
      <DialogContent>
        ...
        <Box sx={{ display: 'flex', gap: 1, my: 2 }}>
          ...
          <Button disabled={!decisions.length} onClick={() => downloadTextFile(filename, JSON.stringify(personalDataFile('crosscheck-decisions', decisions, scope), null, 2), 'application/json')}>Export JSON ({decisions.length})</Button>
        </Box>
        ...
        <List>{decisions.map(entry => <ListItem key={JSON.stringify([entry.dblpKey, entry.halDocid])} sx={{ gap: 2 }}>
          ...
          <Button color="error" onClick={() => remove(entry)}>Undo</Button>
        </ListItem>)}</List>
      </DialogContent>
