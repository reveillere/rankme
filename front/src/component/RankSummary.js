...
export function RankSummary({ records, ranks, selected, sharedMaps }) {
  const activeRanks = Object.entries(ranks).filter(([key]) => selected[key]);

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', minWidth: '160px' }}>
      {activeRanks.map(([key, value]) => {
        const count = records.filter(record => getEffectiveValue(record.rank, sharedMaps?.[portalFromRank(record.rank)]) === key).length;
        return (
